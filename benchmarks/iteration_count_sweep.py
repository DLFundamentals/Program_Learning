from __future__ import annotations

import argparse
import json
from pathlib import Path

from benchmarks.common import (
    add_common_arguments,
    jobs_from_conditions,
    resolve_sweep_artifact_root,
    run_sweep,
    selected_targets,
)
from scripts.plot_iteration_best_runtime_so_far import write_iteration_best_runtime_so_far_plots
from dasbench.utils import timestamp_token

PROFILE_PATH = Path(__file__).with_name("second_scale_v2_profile.json")
DEFAULT_ITERATIONS = 5


def _load_profile() -> dict[str, object]:
    return json.loads(PROFILE_PATH.read_text(encoding="utf-8"))


def build_conditions(
    *,
    train_size: int,
    validation_size: int,
    test_size: int,
    targets: list[tuple[str, str]],
    iterations: int,
    candidate_width: int,
    beam_width: int,
) -> list[dict[str, object]]:
    profile = _load_profile()
    metadata = profile.get("metadata", {})
    instance_params_by_target = profile.get("instance_params_by_target", {})
    if not isinstance(metadata, dict) or not isinstance(instance_params_by_target, dict):
        raise ValueError(f"Invalid second-scale v2 profile file: {PROFILE_PATH}")

    missing_targets = [
        f"{problem}/{family}"
        for problem, family in targets
        if f"{problem}/{family}" not in instance_params_by_target
    ]
    if missing_targets:
        joined = ", ".join(sorted(missing_targets))
        raise ValueError(f"Missing second-scale v2 size profile for: {joined}")

    effective_beam_width = min(beam_width, candidate_width)
    return [
        {
            "condition_id": f"iterations_{iterations:02d}",
            "train_size": train_size,
            "validation_size": validation_size,
            "test_size": test_size,
            "iterations": iterations,
            "beam_width": effective_beam_width,
            "candidate_width": candidate_width,
            "generated_candidate_width": candidate_width,
            "survivor_beam_width": effective_beam_width,
            "skip_baselines": True,
            "skip_report": True,
            "profile_name": "seconds_scale_v2",
            "profile_path": str(PROFILE_PATH),
            "calibration_run_id": metadata.get("calibration_run_id"),
            "calibration_target": metadata.get("calibration_target"),
            "profile_notes_by_target": metadata.get("notes_by_target", {}),
            "instance_params_by_target": instance_params_by_target,
        }
    ]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run an agent-only iteration-count sweep over representative benchmark families."
    )
    add_common_arguments(parser)
    parser.add_argument("--all-families", action="store_true")
    parser.add_argument("--candidate-width", type=int, default=3)
    parser.add_argument("--train-size", type=int, default=64)
    parser.add_argument("--validation-size", type=int, default=32)
    parser.add_argument("--test-size", type=int, default=500)
    parser.set_defaults(repeats=1, iterations=DEFAULT_ITERATIONS)
    return parser


def build_jobs(args: argparse.Namespace, *, sweep_id: str | None = None) -> list:
    sweep_id = sweep_id or args.sweep_id or timestamp_token()
    if args.iterations != DEFAULT_ITERATIONS:
        raise ValueError(
            f"iteration_count_sweep is fixed to {DEFAULT_ITERATIONS} iterations; "
            f"received `--iterations {args.iterations}`."
        )
    targets = selected_targets(
        args.problem,
        args.family,
        representative_only=not args.all_families,
    )
    conditions = build_conditions(
        train_size=args.train_size,
        validation_size=args.validation_size,
        test_size=args.test_size,
        targets=targets,
        iterations=DEFAULT_ITERATIONS,
        candidate_width=args.candidate_width,
        beam_width=args.beam_width,
    )
    return jobs_from_conditions(
        sweep_id=sweep_id,
        artifact_root=resolve_sweep_artifact_root(args.output_root, "iteration_count_sweep", sweep_id),
        targets=targets,
        conditions=conditions,
        args=args,
    )


def main(argv: list[str] | None = None) -> int:
    parser = build_parser()
    args = parser.parse_args(argv)
    sweep_id = args.sweep_id or timestamp_token()
    jobs = build_jobs(args, sweep_id=sweep_id)
    artifact_root = resolve_sweep_artifact_root(args.output_root, "iteration_count_sweep", sweep_id)
    summary = run_sweep(
        sweep_id=sweep_id,
        sweep_kind="iteration_count_sweep",
        jobs=jobs,
        output_root=Path(args.output_root),
        max_workers=args.max_workers,
        dry_run=args.dry_run,
    )
    if not args.dry_run:
        plot_paths = write_iteration_best_runtime_so_far_plots(
            sweep_root=artifact_root,
            condition_id=f"iterations_{DEFAULT_ITERATIONS:02d}",
        )
        for path in plot_paths:
            print(f"Runtime plot: {path}")
    return 1 if summary["failed_count"] else 0


if __name__ == "__main__":
    raise SystemExit(main())
